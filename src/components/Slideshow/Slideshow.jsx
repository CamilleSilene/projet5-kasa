...
const SlideShow = (props) => {
  ...
  return (
    ...
    <div className="SlideShow">
      {props.slides.length > 1 && (<div className="slideClick fa-solid fa-chevron-left"onClick={prevIndex} role="button"></div>
      )}
      <div className="slidesWrapper">
        <img key={currentIndex} src={props.slides[currentIndex]} alt="photos" />
        {props.slides.length > 1 && (<div className="slideshow-pagination">{currentIndex + 1}/{props.slides.length}</div>)}
      </div>
      {props.slides.length > 1 && (<div className="slideClick fa-solid fa-chevron-right" onClick={nextIndex} role="button"></div>
      )}
    </div>
  );
};
